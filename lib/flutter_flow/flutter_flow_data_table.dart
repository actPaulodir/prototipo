...
class FlutterFlowDataTableController<T> extends DataTableSource {
  ...
  List<T> get selectedData => selectedRows.where((i) => i < data.length).map(data.elementAt).toList();
        ...
        ? _dataRowBuilder!(
            ...
            _selectable
                ? (selected) {
                    ...
                    selected ? selectedRows.add(index) : selectedRows.remove(index);
                    notifyListeners();
                  }
                : null,
          )
...
class FlutterFlowDataTable<T> extends StatefulWidget {
  ...
  @override
  State<FlutterFlowDataTable<T>> createState() => _FlutterFlowDataTableState<T>();
}
...
class _FlutterFlowDataTableState<T> extends State<FlutterFlowDataTable<T>> {
  ...
  int get initialRowsPerPage => rowCount > _kMinRowsPerPage ? defaultRowsPerPage : _kMinRowsPerPage;
  ...
  Widget build(BuildContext context) {
    ...
    final checkboxThemeData = CheckboxThemeData(
      ...
      fillColor: WidgetStateProperty.resolveWith(
        (states) => states.contains(WidgetState.selected) ? widget.checkboxSelectedFillColor ?? Colors.white.withOpacity(0.01) : widget.checkboxUnselectedFillColor ?? Colors.white.withOpacity(0.01),
      ),
      side: WidgetStateBorderSide.resolveWith(
        (states) => BorderSide(
          width: 2.0,
          color: states.contains(WidgetState.selected) ? widget.checkboxSelectedBorderColor ?? Colors.black54 : widget.checkboxUnselectedBorderColor ?? Colors.black54,
        ),
      ),
      ...
      child: SizedBox(
        ...
        child: Theme(
          data: Theme.of(context).copyWith(
            iconTheme: widget.sortIconColor != null ? IconThemeData(color: widget.sortIconColor) : null,
          ),
          child: PaginatedDataTable2(
            source: controller,
            controller: widget.paginated ? controller.paginatorController : null,
            rowsPerPage: widget.paginated ? initialRowsPerPage : rowCount,
            availableRowsPerPage: const [5, 10, 25, 50, 100],
            onPageChanged: widget.onPageChanged != null ? (index) => widget.onPageChanged!(index) : null,
            ...
            border: TableBorder(
              ...
              verticalInside: widget.addVerticalDivider
                  ...
                  : BorderSide.none,
              bottom: widget.addTopAndBottomDivider ? horizontalBorder : BorderSide.none,
            ),
  ...
  double _getColumnsWidth(List<DataColumn> columns) => columns.where((c) => c is DataColumn2 && c.fixedWidth != null).fold(
